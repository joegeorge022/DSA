# Data Structures and Algorithms: Lab Experiments
...
10. **Water Jug Problem:**  
    We have three containers whose sizes are 10 liters, 7 liters, and 4 liters, respectively.  
    ...
    We want to know if there is a sequence of pourings that leaves exactly 2 liters in the 7-litre or 4-litre container.  
...
14. **Customer Prioritization in a Post Office:**  
    The General post office wishes to give preferential treatment to its customers. They have identified the customer categories as:  
    - Defence personnel  
    - Differently abled  
    - Senior citizen  
    - Ordinary  
  
    The customers are to be given preference in the decreasing order:  
    - Differently abled  
    - Senior citizen  
    - Defence personnel  
    - Ordinary  
...
15. **Implement a spell checker using a hash table to store a dictionary of words for fast lookup.**  
    Implement functions to check if a word is valid and to suggest corrections for misspelled words.

16. **Simulation of a basic memory allocator and garbage collector using a doubly linked list.**

17. **Tech Fest Activity Points:**  
    The CSE dept is organizing a tech fest with exciting events. By participating in an event, you can claim activity points as stipulated by KTU.  
    Each event i gives you `A[i]` activity points, where `A` is an array.  
    If you are not allowed to participate in more than `k` events, what's the max number of points you can earn?

18. **Merge K Sorted Lists into a Single Sorted List Using a Heap:**  
